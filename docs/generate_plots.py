...
if 'physics/entered_growth' in df.columns:
    ...
    ax.axhline(y=0.25, color=ACCENT2, linestyle=':', linewidth=1.5, alpha=0.7, label='Baseline rate (~25%)')
    ax.set_xlabel('Training Step')
    ax.set_ylabel('Growth Phase Entry Rate (rolling 50)')
    ax.set_title('Growth Phase Discovery — From 25% (Baseline) to 90%+ (Trained)', fontsize=14, fontweight='bold')
...
if 'physics/entered_growth' in df.columns:
    ...
    ax.axhline(y=0.25, color=ACCENT2, linestyle=':', linewidth=1.5, alpha=0.7)
    ax.set_ylim(-0.05, 1.1)
